...
class DefaultLoader(data.Dataset):
    ...
    def __list_dirs(self, root_dir, dataset):
        ...
        elif dataset == "train" and self.configer.exists('data', 'sparse_label_dir'):
            # for sparsely labeled seg
            label_dir = os.path.join(root_dir, dataset, self.configer.get('data', 'sparse_label_dir'))
            Log.info('For Sparsely Labeled SemSeg Training!!!')
            Log.info('label_dir: {}'.format(label_dir))
