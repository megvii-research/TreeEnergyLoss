...
def collate(batch, trans_dict):
    ...
    for i in range(len(batch)):
        ...
        if trans_dict['align_method'] in ['only_scale', 'scale_and_pad']:
            ...
            if 'gtmap' in data_keys:
                gtmap = batch[i]['gtmap'].data.unsqueeze(0).unsqueeze(0).float()
                gtmap = F.interpolate(gtmap, scaled_size_hw, mode='nearest').long().squeeze(0).squeeze(0)
                batch[i]['gtmap'] = DataContainer(gtmap, stack=True)
        ...
        if pad_width > 0 or pad_height > 0:
            ...
            if 'labelmap' in data_keys:
                batch[i]['labelmap'] = DataContainer(F.pad(batch[i]['labelmap'].data, pad=pad, value=-2), stack=batch[i]['labelmap'].stack)
            if 'gtmap' in data_keys:
                batch[i]['gtmap'] = DataContainer(F.pad(batch[i]['gtmap'].data, pad=pad, value=-1),
                                                  stack=batch[i]['gtmap'].stack)
            if 'maskmap' in data_keys:
