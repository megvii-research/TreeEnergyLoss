...
class ADE20KEvaluator(object):
    ...
    def evaluate(self, pred_dir, gt_dir):
        ...
        for filename in os.listdir(pred_dir):
            ...
            if "pascal_context" in gt_dir or "ade" or "ADE" in gt_dir or "coco_stuff" in gt_dir:
            ...
            # print("predmap min_max:", np.min(predmap), np.max(predmap))
            # print("gtmap min_max:", np.min(gtmap), np.max(gtmap))
            # print("predmap val:", np.unique(predmap))
            # print("gtmap val:", np.unique(predmap))
