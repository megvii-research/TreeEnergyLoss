...
class PascalContextEvaluator(object):
    ...
    def evaluate(self, pred_dir, gt_dir):
        ...
        for filename in os.listdir(pred_dir):
            ...
            # print("gtmap path:", gt_path)

            predmap = self.relabel(predmap)
            gtmap = self.relabel(gtmap)

            # print("num class:", self.configer.get('data', 'num_classes'))
            # # print("gtmap:", gtmap)
            # print("predmap value:", np.unique(predmap))
            # print("gtmap value:", np.unique(gtmap))
            # print("predmap gtmap shapes:", predmap.shape, gtmap.shape)
            # print("gtmap shape:", gtmap.shape)
        ...
        Log.info('cls IoU: {}'.format(self.seg_running_score.get_cls_iou()))
