...
class HRNet_W48(nn.Module):
    ...
    def __init__(self, configer):
        ...
        # extra added layers for tree energy loss
        mid_channel = 256
        self.embedding_layer = nn.Sequential(
            nn.Conv2d(in_channels, mid_channel, kernel_size=1, stride=1, padding=0, bias=False),
            ModuleHelper.BNReLU(mid_channel, bn_type=self.configer.get('network', 'bn_type'))
        )

    def forward(self, x):
        _, _, org_h, org_w = x.size()
        ...
        f4 = F.interpolate(x[3], size=(h, w), mode="bilinear", align_corners=True)
        feat = torch.cat([f1, f2, f3, f4], 1)

        out = self.cls_head(feat)  # [b, n, h, w]

        outs = []
        outs.append(F.interpolate(out, size=(org_h, org_w), mode="bilinear", align_corners=True))
